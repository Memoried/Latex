...
密度 $\rho(x)$ 划分为两个子集\cite{nong2025anomaly}：
...
（3）尾部丢失：长尾集合 $\mathcal{T}$ 的合法流量被排斥在边界之外，导致极高的误报率。
...
\subsection{模型整体框架设计}
在明确了欧氏距离（IED）作为多样性度量指标的基础上，本节构建基于实例加权的自监督超球体学习模型。
...
特征学习模块（TFLM）：针对网络流量中非平稳噪声干扰与长序列建模效率低的问题，本
模块引入小波变换与分块机制。原始流量经离散小波变换解耦
为全局低频与局部高频分量后，被划分为序列块并输入双分支异常注意力网络进行
特征提炼。最终输出的低维特征向量（$\mathbf{z}$），为后续计算样本稀缺度及
优化超球体边界提供高质量的基础数据。

多样性评估模块（DAM）：在每个训练批次中，利用 IED 指标实时评估样本的局
部稀缺性，识别出处于分布长尾区域的正常模式。

损失与权重模块（LWM）：根据 IED 指标生成的权重因子 ，对损失函数进行加权更新，修正特征
中心偏移并优化超球体边界。

中心建模模块（CMM）：利用上一步生成的实例权重 $\mathbf{w}$ 计算当前批次的加权质心，并通过指数滑动平均
...
（3）低频驱动的序列关联：低频分量 $E_{low}$ 滤除了噪声突变，最适合刻画全
局平稳演化规律。将其映射为查询、键、值矩阵（$Q, K, V$）：
\begin{equation}
    Q = E_{low} W_Q, \quad K = E_{low} W_K, \quad V = E_{low} W_V
\end{equation}
通过自注意力机制计算数据驱动的序列关联权重分布：
\begin{equation}
    S_i = \text{Softmax}\left(\frac{Q_i K^T}{\sqrt{d_k}}\right), \quad i \in \{1, 2, \ldots, N\}
\end{equation}

（4）高频能量驱动的先验关联：高频分量 $E_{high}$ 直接反映流量的
突变剧烈程度。利用多层感知机（MLP）将其动态映射为先验高斯分布的尺度参数 $\sigma_i$：
\begin{equation}
    \sigma_i = \text{MLP}(E_{high}^{(i)})
\end{equation}
进而计算第 $i$ 个 Patch 对第 $j$ 个 Patch 的先验权重：
\begin{equation}
    P_{i,j} = \frac{1}{\sqrt{2\pi}\sigma_i} \exp\left(-\frac{|j-i|^2}{2\sigma_i^2}\right)
\end{equation}
当局部出现剧烈高频波动（疑似长尾模式或异常）时，该机制能自适应地输出极小的 $\sigma_i$，迫
使注意力窗口收缩聚焦于突变上下文；而在平稳区域，则适度扩张感受野。

联合特征聚合与输出：将先验分布矩阵 $P$ 与序列分布矩阵 $S$ 聚合，并作用于特征值矩阵 $V$。
随后经多层前馈网络（FFN）与层归一化（LayerNorm）进行深度提炼，WPAT 最终计算出低维紧凑的
隐层特征表示 $\mathcal{Z}$：
\begin{equation}
    \mathcal{Z} = \text{LayerNorm}(\text{FFN}((P + S)V))
\end{equation}
其中 $\mathcal{Z} = \{z_1, z_2, \ldots, z_N\} \in \mathbb{R}^{N \times d_k}$，
此时 $z_i$ 代表一个时间 Patch 的特征。这些具备高模式区分度的特征将作为双向输出：一方面用于多
样性评估模块计算 IED 稀缺度指标，另一方面送入中心建模模块，以优化自适应长尾正常模式的加权超球体决策边界。
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.8]{fig/chapter3/中心建模框架.pdf}
	\caption{中心建模模块}
	\label{fig:中心建模模块}
\end{figure}
...
\subsection{实验数据集}
为了全面验证模型在多中心运维数据及复杂网络环境下的泛化能力与鲁棒性，
...
噪声干扰、数据丢失及非逻辑性数值等问题。为了消除数据质量对 IED
 ...
 \begin{figure}[h]
	\centering
	\includegraphics[scale=0.7]{fig/chapter3/数据预处理流程.pdf}
	\caption{数据预处理流程}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.53]{fig/chapter3/t-SNE.pdf}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4]{fig/chapter3/Figure_1.png}
